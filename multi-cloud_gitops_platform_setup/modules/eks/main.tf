...
module "vpc" {
  ...
  private_subnets       = [for i, az in slice(data.data.aws_availability_zones.available.names, 0, 3) : cidrsubnet(var.vpc_cidr, 4, i)]
  public_subnets        = [for i, az in slice(ddata.aws_availability_zones.available.names, 0, 3) : cidrsubnet(var.vpc_cidr, 4, i + 3)]
  ...
  public_subnet_tags = {
    "kubernetes.io/role/elb" = 1  # tags required for AWS Load Balancer intergration
  }
...
resource "aws_eks_cluster" "this" {
  ...
  vpc_config {
    subnet_ids              = concat(module.vpc.public_subnets, module.vpc.private_subnets)  # We include both public and private subnets. For production, you might restrict to private only.
